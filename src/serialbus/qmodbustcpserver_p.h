...
public:
  ...
  QModbusResponse forwardProcessRequest(const QModbusRequest &r,
    ...
    if (q->value(QModbusServer::DeviceBusy).value<quint16>() == 0xffff) {
      ...
      return QModbusExceptionResponse(
          r.functionCode(), QModbusExceptionResponse::ServerDeviceBusy);
    }
  ...
  */
  bool matchingServerAddress(quint8 unitId) const {
    ...
    // qCDebug(QT_MODBUS) << "(TCP server) Wrong server unit identifier address,
    // expected"
  ...
  void setupTcpServer() {
    ...
    QObject::connect(
        m_tcpServer, &QTcpServer::newConnection, q_func(), [this]() {
          Q_Q(QModbusTcpServer);
          ...
          qCDebug(QT_MODBUS)
              << "(TCP server) Incoming socket from" << socket->peerAddress()
              << socket->peerName() << socket->peerPort();

          if (m_observer && !m_observer->acceptNewConnection(socket)) {
            qCDebug(QT_MODBUS)
                << "(TCP server) Connection rejected by observer";
            socket->close();
            socket->deleteLater();
            return;
          }
          ...
          QObject::connect(
              socket, &QTcpSocket::readyRead, q, [buffer, socket, this]() {
                if (!socket)
                  return;
                ...
                while (!buffer->isEmpty()) {
                  qCDebug(QT_MODBUS_LOW).noquote()
                      << "(TCP server) Read buffer: 0x" + buffer->toHex();

                  if (buffer->size() < mbpaHeaderSize) {
                    qCDebug(QT_MODBUS) << "(TCP server) MBPA header too short. "
                  ...
                  qCDebug(QT_MODBUS_LOW)
                      << "(TCP server) Request MBPA:" << "Transaction Id:"
                      << Qt::hex << transactionId
                      << "Protocol Id:" << protocolId
                      << "PDU bytes:" << bytesPdu << "Unit Id:" << unitId;

                  // The length field is the byte count of the following fields,
                  // including the Unit Identifier and the PDU, so we remove on
                  // byte.
                  bytesPdu--;
                  ...
                  if (buffer->size() < current) {
                    qCDebug(QT_MODBUS)
                        << "(TCP server) PDU too short. Waiting for more data";
                    return;
                  }
                  ...
                  qCDebug(QT_MODBUS)
                      << "(TCP server) Request PDU from local port "
                      << remoteSocketPort << " : " << request;
                  const QModbusResponse response =
                      forwardProcessRequest(request, remoteSocketPort);
                  qCDebug(QT_MODBUS)
                      << "(TCP server) Response PDU:" << response;

                  QByteArray result;
                  QDataStream output(&result, QIODevice::WriteOnly);
                  // The length field is the byte count of the following fields,
                  // including the Unit Identifier and PDU fields, so we add one
                  // byte to the response size.
                  output << transactionId << protocolId
                         << quint16(response.size() + 1) << unitId << response;

                  if (!socket->isOpen()) {
                    qCDebug(QT_MODBUS)
                        << "(TCP server) Requesting socket has closed.";
                    forwardError(
                        QModbusTcpServer::tr("Requesting socket is closed"),
                  ...
                  if (writtenBytes == -1 || writtenBytes < result.size()) {
                    qCDebug(QT_MODBUS) << "(TCP server) Cannot write requested "
                                          "response to socket.";
                    forwardError(QModbusTcpServer::tr(
                                     "Could not write response to client"),
                                 QModbusDevice::WriteError);
                  }
                }
              });
    ...
    QObject::connect(m_tcpServer, &QTcpServer::acceptError, q_func(),
                     [this](QAbstractSocket::SocketError /*sError*/) {
                       ...
                       q->setError(m_tcpServer->errorString(),
